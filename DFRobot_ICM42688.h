...
#define TAP_DOUBLE 16
   ...
   * @n
   * ------------------------------------------------------------------------------------------------------------------------------------------
   * @n        | Reserved | DMP_INIT_EN  |  DMP_MEM_RESET_EN |      Reserved |
   * ABORT_AND_RESET   |   TMST_STROBE   |   FIFO_FLUSH   |  Reserved  |
   * @n
   * ------------------------------------------------------------------------------------------------------------------------------------------
   * @n        DMP_INIT_EN: When this bit is set to 1, the DMP is enabled
   * @n        DMP_MEM_RESET_EN: When this bit is set to 1, the DMP memory is
   * reset
   * @n        ABORT_AND_RESET : When this bit is set to 1, the signal path is
   * reset by restarting the ODR counter and signal path controls
   * @n        TMST_STROBE : When this bit is set to 1, the time stamp counter
   ...
   * @n
   * -----------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        |     DMP_POWER_SAVE     |   TAP_ENABLE   |     PED_ENABLE     |
   * TILT_ENABLE    |        R2W_EN      |    Reserved    |             DMP_ODR
   * |
   * @n
   * -----------------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   * @n                    1: Tap Detection enabled when accelerometer ODR is
   * set to one of the ODR
   * @n                    values supported by Tap Detection (200Hz, 500Hz,
   * 1kHz)
   ...
   * @n
   * ----------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        ACCEL_FS_SEL :Full scale select for accelerometer UI interface
   * output
   * @n                      000: ±16g (default)
   * @n                      001: ±8g
   * @n                      010: ±4g
   ...
   * ----------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        |                          TEMP_FILT_BW                         |
   * reserved       |            GYRO_UI_FILT_ORD         | GYRO_DEC2_M2_ORD |
   * @n
   * ----------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        TEMP_FILT_BW : Set the bandwidth of the temperature signal DLPF
   * @n                      000: DLPF BW = 4000Hz; DLPF Latency = 0.125ms
   * (default)
   ...
   * ----------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        |                    Reserved              |      TEMP_DIS      |
   * IDLE        |                GYRO_MODE            |           ACCEL_MODE |
   * @n
   * ----------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        TEMP_DIS : 0: Temperature sensor is enabled (default)
   * @n                   1: Temperature sensor is disabled
   * @n        IDLE : 0: When the accelerometer and gyroscope are powered off,
   * the chip will go to OFF state, since the RC oscillator will also be powered
   * off
   * @n               1: RC oscillator is powered on even if the accelerometer
   * and gyroscope are powered off
   ...
   * @n                   Gyroscope needs to be kept ON for a minimum of 45ms.
   ...
   * @n
   * ------------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        | FIFO_HOLD_LAST_DATA_EN | FIFO_COUNT_REC  |  FIFO_COUNT_ENDIAN |
   * SENSOR_DATA_ENDIAN |                   Reserved            | UI_SIFS_CFG |
   * @n
   * ------------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        FIFO_HOLD_LAST_DATA_EN: Method for processing invalid data, 0:
   * Registers do not receive invalid samples. Registers hold last valid sample
   * until new one arrives
   * @n                                                           FIFO : 16-bit
   * FIFO packet: Same as Sense Registers (Do not receive invalid samples. They
   * hold the last valid sample. Repeated reading before new sample received
   * will yield copies of the last valid sample)
   * @n                                                                  20-bit
   * FIFO packet: Invalid samples are indicated with the value -524288
   * @n                                                        1: Not receive
   * invalid samples. Registers hold last valid sample until new one arrives
   * @n        FIFO_COUNT_REC: 0: FIFO count is reported in bytes
   * @n                        1: FIFO count is reported in records
   * @n        FIFO_COUNT_ENDIAN : 0: FIFO count is reported in Little Endian
   * format
   * @n                            1: FIFO count is reported in Big Endian
   * format (default)
   * @n        SENSOR_DATA_ENDIAN : 0: Sensor data is reported in Little Endian
   * format
   * @n                             1: Sensor data is reported in Big Endian
   * format (default)
   ...
   * ----------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        |                                       Reserved |
   * ACCEL_LP_CLK_SEL  |    RTC_MODE    |              CLKSEL         |
   * @n
   * ----------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        ACCEL_LP_CLK_SEL : 0: Accelerometer LP mode uses Wake Up
   * oscillator clock
   * @n                           1: Accelerometer LP mode uses RC oscillator
   * clock
   ...
   * @n                 01: Select PLL when available, else select RC oscillator
   * (default)
   * @n                 10: Reserved
   ...
   * ----------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        |                            Reserved                           |
   * ACCEL_UI_FILT_ORD              |         ACCEL_DEC2_M2_ORD       | Reserved
   * |
   * @n
   * ----------------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   * @n                              8 to 13: Reserved
   * @n                              14 Low Latency option: Trivial decimation @
   * ODR of Dec2 filter output. Dec2
   ...
   * @n                            8 to 13: Reserved
   * @n                            14 Low Latency option: Trivial decimation @
   * ODR of Dec2 filter output. Dec2 runs at max(400Hz, ODR)
   * @n                            15 Low Latency option: Trivial decimation @
   ...
   * @n
   * -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        TAP_MIN_JERK_THR  Tap detection minimum jerk threshold, use
   * default value 010001b
   ...
   * @n
   * -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        |        Reserved      |                   TAP_TMAX | TAP_TAVG |
   * TAP_TMIN                           |
   * @n
   * -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        TAP_TMAX  Tap measurement window (number of samples), use default
   * value 01b
   * @n        TAP_TAVG  Tap energy measurement window (number of samples), use
   * default value 01b
   ...
   * @n
   * -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        |           TILT_WAIT_TIME_SEL                | SLEEP_TIME_OUT |
   * Reserved                        |
   * @n
   * -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        TILT_WAIT_TIME_SEL  Config delay time after tilt is detected
   * before interrupt is triggered
   * @n                            00: 0s
   * @n                            01: 2s
   * @n                            10: 4s (default)
   * @n                            11: 6s
   * @n        SLEEP_TIME_OUT  Configures the time for sleep detection, for
   * Raise to Wake/Sleep
   * @n                        000: 1.28sec
   * @n                        001: 2.56sec
   * @n                        010: 3.84sec
   ...
   * @n
   * -----------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        |                                    Reserved |     WOM_INT_MODE
   * |    WOM_MODE    |             SMD_MODE            |
   * @n
   * -----------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        WOM_INT_MODE : (Wake on motion) 0: Set WoM interrupt on the OR of
   * all enabled accelerometer thresholds
   * @n                                 1: Set WoM interrupt on the AND of all
   * enabled accelerometer threshold
   * @n        WOM_MODE 0: Initial sample is stored. Future samples are compared
   * to initial sample
   * @n                 1: Compare current sample to previous sample
   * @n        SMD_MODE  00: SMD disabled (important motion detector)
   * @n                  01: Reserved
   * @n                  10: SMD short (1 sec wait) An SMD event is detected
   * when two WOM are detected 1 sec apart
   ...
   * -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        |                  Reserved                   |
   * GYRO_Z_NF_COSWZ_SEL[0] | GYRO_Y_NF_COSWZ_SEL[0] | GYRO_X_NF_COSWZ_SEL[0] |
   * GYRO_Z_NF_COSWZ[8] | GYRO_Y_NF_COSWZ[8] | GYRO_X_NF_COSWZ[8] |
   * @n
   * -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        GYRO_Z_NF_COSWZ_SEL[0] :Used for gyroscope Z-axis notch filter
   * frequency selection
   * @n        GYRO_Y_NF_COSWZ_SEL[0] :Used for gyroscope Y-axis notch filter
   * frequency selection
   * @n        GYRO_X_NF_COSWZ_SEL[0] :Used for gyroscope X-axis notch filter
   * frequency selection
   * @n        GYRO_Z_NF_COSWZ[8] :Used for gyroscope Z-axis notch filter
   * frequency selection
   * @n        GYRO_Y_NF_COSWZ[8] :Used for gyroscope Y-axis notch filter
   * frequency selection
   ...
   * @n
   * ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n         GYRO_AAF_BITSHIFT :Controls bandwidth of the gyroscope
   * anti-alias filter
   ...
   * @n
   * -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        ACCEL_AAF_DELT  Controls bandwidth of the accelerometer
   * anti-alias filter
   * @n        ACCEL_AAF_DIS   0: Enable accelerometer anti-aliasing filter
   * (default)
   ...
   * @n
   * -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        |                                  ACCEL_AAF_BITSHIFT |
   * ACCEL_AAF_DELTSQR[11:8]                                 |
   * @n
   * -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        ACCEL_AAF_BITSHIFT  Controls bandwidth of the accelerometer
   * anti-alias filter
   ...
   * @n
   * ------------------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        |       Reserved       | FIFO_RESUME_PARTIAL_RD | FIFO_WM_GT_TH
   * |   FIFO_HIRES_EN   | FIFO_TMST_FSYNC_EN |  FIFO_TEMP_EN  |  FIFO_GYRO_EN
   * | FIFO_ACCEL_EN |
   * @n
   * ------------------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        FIFO_RESUME_PARTIAL_RD : 0: Partial FIFO read is disabled and the
   * entire FIFO needs to be re-read
   * @n                                 1: FIFO read can be partial, and resume
   * from last read point
   * @n        FIFO_WM_GT_TH Trigger FIFO watermark interrupt on every ODR (DMA
   * write) if FIFO_COUNT ≥ FIFO_WM_TH
   * @n        FIFO_HIRES_EN : FIFO_HIRES_EN Enable 3 bytes of extended 20-bits
   * accel, gyro data  +  1 byte of extended 16-bit temperature sensor data to
   * be placed into the FIFO
   * @n        FIFO_TMST_FSYNC_EN: Must be set to 1 for all FIFO use cases when
   * FSYNC is used.
   ...
   * -----------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        |                            Reserved                           |
   * TMST_TO_REGS_EN  |       TMST_RES     |  TMST_DELTA_EN  |  TMST_FSYNC_EN  |
   * TMST_EN  |
   * @n
   * -----------------------------------------------------------------------------------------------------------------------------------------------------------
   * @n        TMST_TO_REGS_EN : 0: TMST_VALUE[19:0] read always returns 0s
   * @n                          1: TMST_VALUE[19:0] read returns timestamp
   * value
   * @n        TMST_RES : Time Stamp resolution: When set to 0 (default), time
   * stamp resolution is 1 µs. When set to 1, resolution is 16µs
   * @n        TMST_DELTA_EN  Time Stamp delta enable: When set to 1, the time
   * stamp field contains the measurement of time since the last occurrence of
   * ODR.
   * @n        TMST_FSYNC_EN  Time Stamp register FSYNC enable (default). When
   * set to 1, the contents of the Timestamp feature of FSYNC is enabled. The
   * user also needs to select FIFO_TMST_FSYNC_EN in order to propagate the
   * timestamp value to the FIFO.
   ...
   * @n
   * -------------------------------------------------------------------------------------------------------------------------------
   * @n        |      reversed       |    INT2_MODE    | INT2_DRIVE_CIRCUIT |
   * INT2_POLARITY | INT1_MODE | INT1_DRIVE_CIRCUIT | INT1_POLARITY |
   * @n
   * -------------------------------------------------------------------------------------------------------------------------------
   ...
   * @n
   * ---------------------------------------------------------------------------------------------------------------------------------------------
   * @n        |      reversed       |    STEP_DET_INT_EN    |
   * STEP_CNT_OFL_INT_EN | TILT_DET_INT_EN | WAKE_DET_INT_EN | SLEEP_DET_INT_EN
   * | TAP_DET_INT_EN |
   * @n
   * ---------------------------------------------------------------------------------------------------------------------------------------------
   */
 ...
 public:
   ...
   * @n       Z_AXIS_WOM
   * @n       ALL
   * @param threshold  Range(0-255) [WoM thresholds are expressed in fixed “mg”
   * independent of the selected Range [0g : 1g]; Resolution 1g/256=~3.9mg]
   */
   ...
   * @param mode
   * @n      0: disable SMD
   * @n      2 : SMD short (1 sec wait) An SMD event is detected when two WOM
   * are detected 1 sec apart
   * @n      3 : SMD long (3 sec wait) An SMD event is detected when two WOM are
   ...
   * @n      ICM42688_INT_STATUS2    Obtain interrupt information of SMD_INT,
   * WOM_X_INT, WOM_Y_INT, WOM_Z_INT and clear them
   * @n      ICM42688_INT_STATUS3    Obtain interrupt information of
   * STEP_DET_INT, STEP_CNT_OVF_INT, TILT_DET_INT, WAKE_INT, TAP_DET_INT and
   * clear them
  ...
  /**
   * @fn getFIFOData
   * @brief Read FIFO data, read temperature, gyroscope and accelerometer data
   * and save them for parse.
   */
  void getFIFOData();
   ...
   * @n       1  Interrupt lock mode (polarity remains unchanged when interrupt
   * triggered, and restore after clearing interrupt)
   * @n       0  Pulse mode
   ...
   * @n       1  Push pull
   */
  void setINTMode(uint8_t INTPin, uint8_t INTmode, uint8_t INTPolarity,
   ...
   * @n       LP_MODE_ONLY_ACCEL  2     Set low-power mode, only support
   * accelerometer
   * @n       LN_MODE  3                Set low-noise mode
   */
 ...
 protected:
  ...
  /**
   * @fn writeReg
   * @brief Write register function, design it as a virtual function,
  ...
  /**
   * @fn readReg
   * @brief Read register function, design it as a virtual function, implemented
   * by a derived class.
   * @param reg  Register address 8bits
 ...
 private:
   ...
   * @n              uint16_t AAFHZ[63] =
   * {42,84,126,170,213,258,303,348,394,441,488,536,585,634,684,734,785,837,890,943,
   * @n
   * 997,1051,1107,1163,1220,1277,1336,1395,1454,1515,1577,1639,1702,1766,1830,1896,
   * @n
   * 1962,2029,2097,2166,2235,2306,2377,2449,2522,2596,2671,2746,2823,2900,2978,3057,
