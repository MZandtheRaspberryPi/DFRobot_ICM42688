 ...
 * @brief Define basic structure of DFRobot_ICM42688 class, the implementation
 * of basic method
...
int DFRobot_ICM42688::begin(void) {
  ...
  uint8_t reset = 1;
  writeReg(ICM42688_DEVICE_CONFIG, &reset, 0x01);
  sleep_ms(1);
  ...
  //   ICM42688.setODRAndFSR(/* who= */ GYRO, /* ODR= */ ODR_1KHZ,
  //     /* FSR = */ FSR_0);
  // ICM42688.setODRAndFSR(/* who= */ ACCEL, /* ODR= */ ODR_500HZ,
  //     /* FSR = */ FSR_0);
  ...
  writeReg(ICM42688_REG_BANK_SEL, &bank, 1);
  uint8_t gyro_accel_on = 0x0F;
  writeReg(0x4E, &gyro_accel_on, 1);
  sleep_ms(1);

  uint8_t pwr_mgmt0;
  readRandomReg(0, 0x4E, &pwr_mgmt0, 1);
...
uint8_t DFRobot_ICM42688::readRandomReg(uint8_t bank, uint8_t reg,
                                        uint8_t *buff, uint8_t len) {
  writeReg(ICM42688_REG_BANK_SEL, &bank, 1);
  return readReg(reg, buff, len);
}
...
void DFRobot_ICM42688_SPI::writeReg(uint8_t reg, void *pBuf, size_t size) {
  ...
  spi_write_blocking(SPI_PORT, _pBuf, size);
  cs_deselect();
}
...
uint8_t DFRobot_ICM42688_SPI::readReg(uint8_t reg, void *pBuf, size_t size) {
  ...
  reg |= READ_BIT;
  cs_select();
  count = spi_read_blocking(SPI_PORT, reg, _pBuf, size);
  cs_deselect();
